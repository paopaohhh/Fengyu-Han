...
# df_2020 = pd.read_csv('./data/Filtered Indicators/ROE15 net cash ratio0.9.csv')
...
parameters = {'criterion':('gini', 'entropy'),'n_estimators':range(1,20),"min_samples_split":[0.05, 0.1, 0.15, 0.2, 0.25, 0.3]}
...
df_results_ = pd.DataFrame(gs.cv_results_)
...
print('在测试集上，准确率：%.4f， AUC：%.4f，精度：%.4f，召回率：%.4f，F1：%.4f。' \
      % (accuracy_score(y_test, y_predict), roc_auc_score(y_test, y_predict), \
         precision_score(y_test, y_predict), recall_score(y_test, y_predict), f1_score(y_test, y_predict)))

df_result = pd.DataFrame([['Random Forest', results['mean_train_Accuracy'][best_index],results['mean_train_AUC'][best_index],results['mean_train_Precision'][best_index],results['mean_train_Recall'][best_index],results['mean_train_F1'][best_index],\

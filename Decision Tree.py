...
gs = GridSearchCV(
    ...
    param_grid={'criterion':('gini', 'entropy'), 'splitter':('best', 'random'), "min_samples_split":[0.05, 0.1, 0.15, 0.2, 0.25, 0.3]},
    scoring=scoring,
...
df_result = pd.DataFrame([['Decision Tree', results['mean_train_Accuracy'][best_index],results['mean_train_AUC'][best_index],results['mean_train_Precision'][best_index],results['mean_train_Recall'][best_index],results['mean_train_F1'][best_index],\
...
# df_result.to_csv('./筛选数据测试结果.csv',mode='a', index=0, encoding='gbk', header=None)